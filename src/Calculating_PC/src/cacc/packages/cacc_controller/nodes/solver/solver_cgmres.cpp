...
// CACC Controller
Prarx<2*NCAR-1>                                 prarx[NCARMAX];
Platoon<NCAR, Prarx<2*NCAR-1> >                 platoon(prarx);
...
int main(int argc, char** argv)
{
    ...
    if(ncar > NCARMAX/2){
        ...
        case DETECTION:
            ROS_INFO("Solver position_type has been set DETECTION: using distance and position of only automated cars.");
            if(ncar < 1){
            ...
            break;
        case LOCALIZATION:
            ROS_INFO("Solver position_type has been set LOCALIZATION: using localization of whole cars.");
            if(ncar < 2){
            ...
            break;
        case COMBINED:
            ROS_INFO("Solver position_type has been set COMBINED: using both localization and detection of whole cars.");
            if(ncar < 1){
                ROS_ERROR("The number of car has to be bigger than 0.");
                return -1;
            }
            break;
        default:
            ROS_ERROR("The mode has been set with wrong number.");
            return -1;
        }
    }
    ...
    //In case of localization, check the manual flag
        ...
        if(ncar - manual_sum != NCAR){
            ROS_ERROR("The number of cars is not matched with NCAR set in the solver. Please check the launch file or rebuild the solver with correct number.");
            return -1;
        }
    }
    ...
    case DETECTION:
        for(int ic = 0; ic < ncar; ic++){
            ...
            state_packet_listener[ic*2+1] = new StatePacketListener(ic*2+1, id[ic*2+1]);
            curr_state_packet_speaker[ic*2+1] = new StatePacketSpeaker(ic*2+1, id[ic*2+1], StatePacketType::CURRENT);
            state_error_packet_speaker[ic*2+1] = new StatePacketSpeaker(ic*2+1, id[ic*2+1], StatePacketType::ERROR);
            predicted_state_packet_speaker[ic*2+1] = new StatePacketSpeaker(ic*2+1, id[ic*2+1], StatePacketType::PREDICT);
            control_packet_speaker[ic] = new ControlPacketSpeaker(ic+1, id[ic*2+1]);
            
            id[ic*2] = 500 + id[ic*2+1];
            state_packet_listener[ic*2] = new StatePacketListener(ic*2, id[ic*2]); 
            curr_state_packet_speaker[ic*2] = new StatePacketSpeaker(ic*2, id[ic*2], StatePacketType::CURRENT);
            state_error_packet_speaker[ic*2] = new StatePacketSpeaker(ic*2, id[ic*2], StatePacketType::ERROR);
            predicted_state_packet_speaker[ic*2] = new StatePacketSpeaker(ic*2, id[ic*2], StatePacketType::PREDICT); 
        }
        break;
    ...
    case LOCALIZATION:
        ...
        for(int ic = 0; ic < ncar; ic++){
            if(!manual[ic]){
                ...
                state_packet_listener[2*iac+1] = new StatePacketListener(2*iac+1, id[2*iac+1]); 
                curr_state_packet_speaker[2*iac+1] = new StatePacketSpeaker(2*iac+1, id[2*iac+1], StatePacketType::CURRENT); 
                state_error_packet_speaker[2*iac+1] = new StatePacketSpeaker(2*iac+1, id[2*iac+1], StatePacketType::ERROR); 
                predicted_state_packet_speaker[2*iac+1] = new StatePacketSpeaker(2*iac+1, id[2*iac+1], StatePacketType::PREDICT); 
                ...
                state_packet_listener[2*iac] = new StatePacketListener(2*iac, id[2*iac]);
                curr_state_packet_speaker[2*iac] = new StatePacketSpeaker(2*iac, id[2*iac], StatePacketType::CURRENT);
                state_error_packet_speaker[2*iac] = new StatePacketSpeaker(2*iac, id[2*iac], StatePacketType::ERROR);
                predicted_state_packet_speaker[2*iac] = new StatePacketSpeaker(2*iac, id[2*iac], StatePacketType::PREDICT);
    ...
    while(ros::ok()){
        ...
        if(std::isnan(platoon.u.norm())){
            controller.init(&platoon);
            ROS_WARN("The CACC controller has failed to converge and been initialized.");
        }
