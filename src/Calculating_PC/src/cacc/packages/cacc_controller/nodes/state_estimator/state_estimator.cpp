...
int main(int argc, char** argv)
{
    ...
    case DETECTION: //Create ncar of listener and 2*ncar of speaker
        for(int ic = 0; ic < ncar; ic++){
            ...
            state_packet_speaker[ic*2+1] = new StatePacketSpeaker(ic*2+1, id[ic*2+1]);

            id[ic*2] = 500 + id[ic*2+1];
            state_packet_speaker[ic*2] = new StatePacketSpeaker(ic*2, id[ic*2]);
        }
        break;
    case LOCALIZATION: //Create ncar of listener and 2*(ncar-manual_sum) of speaker
        for(int ic = 0; ic < ncar; ic++){
            if(!manual[ic]){
                ...
                state_packet_speaker[iac*2+1] = new StatePacketSpeaker(iac*2+1, id[iac*2+1]);
                ...
                state_packet_speaker[iac*2] = new StatePacketSpeaker(iac*2, id[iac*2]);
