...
public:
    ...
    SensorPacketListener(int _car, int _id){
        id = _id;
        cacc_sensor_packet_sub = nh.subscribe("/Car_"+std::to_string(_car)+"/cacc_sensor_packet", 1, &SensorPacketListener::caccSensorPacketCallback, this);
    };
...
public:
    ros::NodeHandle nh;
    int id;
    StatePacketSpeaker(int _car, int _id){
        id = _id;
        cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_state_packet", 1);
    };
    StatePacketSpeaker(int _car, int _id, bool _predict){
        id = _id;
        if(_predict) cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_predicted_state_packet", 1);
        else cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_curr_state_packet", 1);
    };
    StatePacketSpeaker(int _car, int _id, StatePacketType _type){
        id = _id;
        switch(_type){
        case NORMAL:
            cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_state_packet", 1);
            break;
        case ERROR:
            cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_error_packet", 1);
            break;
        case CURRENT:
            cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_curr_state_packet", 1);
            break;
        case PREDICT:
            cacc_state_packet_pub = nh.advertise<cacc_msgs::CaccStatePacket>("/CACC_Car_"+std::to_string(_car)+"/cacc_predicted_state_packet", 1);
            break;
        }
    };
    void publish_cacc_state_packet_msg(const cacc_msgs::CaccStatePacket& msg){
        ...
        if(cacc_state_packet_msg.id != id){
            ROS_ERROR("Incorrect ID on state packet has been set to StatePacketSpeaker. ID of the message is not matched to the speaker ID.");
        }
        else{
...
public:
    ros::NodeHandle nh;
    int id;
    StatePacketListener(int _car, int _id){
        id = _id;
        cacc_state_packet_sub = nh.subscribe("/CACC_Car_"+std::to_string(_car)+"/cacc_state_packet", 1, &StatePacketListener::caccStatePacketCallback, this);
    };
