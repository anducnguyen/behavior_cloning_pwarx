...
public:
	...
	virtual void cal_output(const x_t &in_x, const int i, double* out_y) = 0;
	virtual void cal_dt(const x_t &in_x, const int i, double* out_dt) = 0;
	virtual void cal_dfdx(const x_t &in_x, const int i, x_t* out_dfdx) = 0;
};
		...
		for(int i = 0; i < NCar; i++){
			...
			if(manual[i]){
				...
				if(activated[j]){
					...
					driver_model[i]->cal_dfdx(_x, i, &M);
					lprime += -lmd[3*i+2]*M;
				}
				else{
					lprime[3*i+0] = 0;
