...
double y_offset = 0.0;
double THW;
double joy_speed_control = 0.0;
double joy_y_e_control = 0;
...
{
    joy_speed_control = 0.5 * joy_msg.axes[1];

    if(joy_msg.buttons[0] == 1){
        if(joy_y_e_control < 0.5)
        joy_y_e_control += 1.0;
    }
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()) {
        ...
        //change speed ref by joystick
        speed_ref += joy_speed_control;
