...
int Flag_motor_start = 1;
bool Flag_control_start = true;
bool Flag_csv_output = true;
bool Flag_speed_ref_update = false;
bool Flag_y_shift = false;

std::string imu_topic, current_pose_topic, motor_signal_topic;
std::string control_cmd_topic, filtered_imu_topic, speed_ref_topic, curr_speed_topic;
...
{
  if (joy_msg.axes[1] >= 0)
    dat.accel = 15 * joy_msg.axes[1];
  else 
    dat.accel = -60 + 15 * joy_msg.axes[1];

  if (iii == 0)
    dat.steer = 150 * joy_msg.axes[2];
}
...
int main(int argc, char **argv)
{
    ...
    pn.param<std::string>("path_file", path_file, "/home/thori/test_cacc/src/Calculating_PC/src/controller_calc/Path_Info/Test_resampled_path.csv");
    // path_file = "/home/thori/test_cacc/src/Calculating_PC/src/controller_calc/Path_Info/Test_resampled_path.csv";
    //pn.param<std::string>("path_to_write_path", path_to_write_path, "src/controller_calc/Path_Info/resampled_path.csv");
    pn.param<std::string>("path_to_write_path", path_to_write_path, "/home/thori/test_cacc/src/Calculating_PC/src/controller_calc/Path_Info/Test_resampled_path.csv");
    // pn.param<std::string>("output_lane_for_debug", output_lane_for_debug, "src/controller_calc/Path_Info/output_lane.csv");
    output_lane_for_debug = "/home/thori/log/output_lane.csv";
    ...
    pn.param<bool>("Flag_y_shift", Flag_y_shift, false); 
    ...
    ros::Subscriber sub2 = n.subscribe(motor_signal_topic, 1000, chatterCallback_MotorSignal);
    ros::Subscriber sub3 = n.subscribe(speed_ref_topic, 5, chatterCallback_SpeedRef);

    ros::Subscriber joy_sub = n.subscribe("joy", 50, joy_callback);
    ...
    //    myfile << "Time,px,py,pz,vx,vy,u,y_e,theta_e,theta_d_lane,ahead_theta_d,y_e_dot,theta_e_dot,delta_theta_d_lane,speed_pwm_cmd,steer_pwm_cmd,gyro_x,gyro_y,gyro_z,acc_x,acc_y,acc_z,angle_x,angle_y,angle_z,angle_Optitrack_x,angle_Optitrack_y,angle_Optitrack_z\n";
        myfile << "Time,px,py,pz,vx,vy,u,y_e,theta_e,theta_d_lane,ahead_theta_d,y_e_dot,theta_e_dot,delta_theta_d_lane,dat.accel,steer_pwm_cmd\n";
    }
